...
export default function ReactSidebar({
  ...
  const [openSections, setOpenSections] = useState<string[]>(
    reactSections.length > 0 ? [reactSections[0].id] : []
  );
  ...
  const allTopics = useMemo(
    () => reactSections.flatMap((section) => section.topics),
    []
  );
  ...
  const activeSection = reactSections.find((section) =>
  ...
  const filteredSections = useMemo(() => {
    ...
    return reactSections
      ...
      .filter(Boolean) as typeof reactSections;
  }, [searchTerm]);
